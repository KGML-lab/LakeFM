...
usage() {
  echo "Usage:"
  echo "  bash scripts/eval_lakefm5m.sh <run_name> <lake_name> [depth_m] [--plot] [--depth depth_m] [--denorm] [--gpus N] [--vars JSON_LIST] [--mask-vars JSON_LIST] [--mask-depths JSON_LIST_OR_NUMBER] [--eval-dataset NAME]"
  echo ""
  echo "Notes:"
...
cmd=(
  ...
  model.revin=False
  model.d_model=128
  model.var_embed_dim=64
  model.depth_embed_dim=32
  model.inp_embed_dim=64
  model.time_embed_dim=16
  model.static_dim=32
  model.temporal_dim=96
  model.cl_proj_dim=32
  model.d_ff=2048
  model.dropout_p=0.0
  model.attn_dropout_p=0.0
  model.head_dropout=0.02
  data.ds_plot_id=0
