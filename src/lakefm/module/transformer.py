...
class TransformerDecoderLayer(nn.Module):
    def __init__(
        ...
        norm1: Optional[nn.Module],
        norm2: Optional[nn.Module],
        norm3: Optional[nn.Module],
        norm4: Optional[nn.Module],
        post_attn_dropout_p: float = 0.0,
        pre_norm: bool = True,
    ):
        super().__init__()
        ...
        self.ffn1 = ffn1  # FFN after self-attention
        self.ffn2 = ffn2  # FFN after cross-attention
        self.norm1 = norm1 or nn.Identity()
        self.norm2 = norm2 or nn.Identity()
        self.norm3 = norm3 or nn.Identity()
        self.norm4 = norm4 or nn.Identity()

    def forward(
        self,
        x: Float[torch.Tensor, "*batch t_len dim"],
        memory: Float[torch.Tensor, "*batch s_len dim"],
        self_attn_mask: Optional[Bool[torch.Tensor, "*batch t_len t_len"]] = None,
        cross_attn_mask: Optional[Bool[torch.Tensor, "*batch t_len s_len"]] = None,
        var_id: Optional[Int[torch.Tensor, "*batch t_len"]] = None,
        ...
        if self.pre_norm:
            # Self-attention + FFN (original Transformer pattern)
            x = x + self._sa_block(self.norm1(x), self_attn_mask, var_id, time_id)
            x = x + self.ffn1(self.norm2(x))
            # Cross-attention + FFN (original Transformer pattern)
            x = x + self._ca_block(self.norm3(x), memory, cross_attn_mask, var_id, time_id, mem_var_id, mem_time_id)
            x = x + self.ffn2(self.norm4(x))
        else:
            # Self-attention + FFN (original Transformer pattern)
            x = self.norm1(x + self._sa_block(x, self_attn_mask, var_id, time_id))
            x = self.norm2(x + self.ffn1(x))
            # Cross-attention + FFN (original Transformer pattern)
            x = self.norm3(x + self._ca_block(x, memory, cross_attn_mask, var_id, time_id, mem_var_id, mem_time_id))
            x = self.norm4(x + self.ffn2(x))
        return x

    def _sa_block(self, x, attn_mask, var_id, time_id):
        out = self.self_attn(
            x, x, x,
            attn_mask=attn_mask,
            query_var_id=var_id,
    ...
    def _ca_block(self, x, memory, attn_mask, q_var_id, q_time_id, kv_var_id, kv_time_id):
        out = self.cross_attn(
            x, memory, memory,
            attn_mask=attn_mask,
            query_var_id=q_var_id,
            kv_var_id=kv_var_id,
            query_time_id=q_time_id,
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        # Self-attn components
        var_attn_bias = TransformerEncoder.get_layer(d_model, num_heads, num_groups, var_attn_bias_layer, shared_var_attn_bias)
        time_attn_bias = TransformerEncoder.get_layer(d_model, num_heads, num_groups, time_attn_bias_layer, shared_time_attn_bias)
        var_qk_proj = TransformerEncoder.get_layer(d_model, num_heads, num_groups, var_qk_proj_layer, shared_var_qk_proj)
        time_qk_proj = TransformerEncoder.get_layer(d_model, num_heads, num_groups, time_qk_proj_layer, shared_time_qk_proj)
        ...
        # Cross-attn can share the same construction
        get_cross_attn = get_self_attn
        ...
        get_norm = partial(norm_layer, d_model)
        ...
        self.layers = nn.ModuleList(
            [
                TransformerDecoderLayer(
                    ...
                    ffn1=get_ffn(),  # FFN after self-attention
                    ffn2=get_ffn(),  # FFN after cross-attention
                    norm1=get_norm(),
                    norm2=get_norm(),
                    norm3=get_norm(),
                    norm4=get_norm(),
    ...
    def forward(
        self,
        tgt: Float[torch.Tensor, "*batch t_len dim"],
        memory: Float[torch.Tensor, "*batch s_len dim"],
        self_attn_mask: Optional[Bool[torch.Tensor, "*batch t_len t_len"]] = None,
        cross_attn_mask: Optional[Bool[torch.Tensor, "*batch t_len s_len"]] = None,
        var_id: Optional[Int[torch.Tensor, "*batch t_len"]] = None,
