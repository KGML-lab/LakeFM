...
class LakeEvalDataset(Dataset):
    ...
    def parse_wide_lake_column(self, col_name):
        m = re.match(r"^(?P<var>.+)_(?P<depth>-?\d+(?:\.\d+)?)$", str(col_name))
        if not m:
            return None
        return m.group("var"), float(m.group("depth"))

    def prepare_lake_dataframe_format(self):
        """
        Normalize lake dataframe format to long form:
            [date_col, depth_col, <base 2D vars...>]

        Wide input (date + columns like var_depth), converted to long
        """
        if self.date_col not in self.lake_df.columns:
            raise ValueError(
                f"Lake dataframe must contain date column `{self.date_col}`; "
                f"got columns: {list(self.lake_df.columns)[:10]}..."
            )

        lake_cols = list(self.lake_df.columns)
        existing_name_to_id = {name: vid for name, vid in zip(self.var_names_2D, self.variate_ids_2D)}
        has_depth_col = self.depth_col in lake_cols
        has_long_var_cols = any(v in lake_cols for v in self.var_names_2D) if self.var_names_2D else False

        # Already long-form; keep as-is.
        if has_depth_col and has_long_var_cols:
            self.lake_input_format = "long"
            return

        # Try wide-form detection (columns like `var_depth`).
        wide_meta = []
        for col in lake_cols:
            if col == self.date_col:
                continue
            parsed = self.parse_wide_lake_column(col)
            if parsed is None:
                continue
            var_name, depth_val = parsed
            if self.var_names_2D and var_name not in self.var_names_2D:
                continue
            wide_meta.append((col, var_name, depth_val))

        if len(wide_meta) == 0:
            raise ValueError(
                f"Lake dataframe is neither long-form (`{self.depth_col}` + base vars) "
                f"nor wide-form (`<var>_<depth>` columns). "
                f"Columns sample: {lake_cols[:20]}"
            )

        # Determine active base 2D variables and align names/ids.
        if self.var_names_2D:
            active_vars = [v for v in self.var_names_2D if any(var == v for _, var, _ in wide_meta)]
        else:
            active_vars = sorted({var for _, var, _ in wide_meta})

        aligned_vars = []
        aligned_ids = []
        for var in active_vars:
            if var in existing_name_to_id:
                aligned_vars.append(var)
                aligned_ids.append(int(existing_name_to_id[var]))
            elif var in self.var2id:
                aligned_vars.append(var)
                aligned_ids.append(int(self.var2id[var]))

        if len(aligned_vars) == 0:
            raise ValueError(
                "Could not align 2D variable names/ids for wide lake format. "
                f"Detected vars: {active_vars}, known var2id keys: {list(self.var2id.keys())[:20]}"
            )

        self.var_names_2D = aligned_vars
        self.variate_ids_2D = aligned_ids

        selected_meta = [(c, v, d) for c, v, d in wide_meta if v in set(self.var_names_2D)]
        selected_cols = [c for c, _, _ in selected_meta]
        col_to_var = {c: v for c, v, _ in selected_meta}
        col_to_depth = {c: d for c, _, d in selected_meta}

        # Collect all unique depths from column structure to preserve standardized depth grid
        all_depths_from_columns = sorted(set(d for _, _, d in selected_meta))
        print(f"\nDEBUG: All depths from column structure: {len(all_depths_from_columns)} depths")
        print(f"  Depths: {all_depths_from_columns}")

        # Convert wide -> long so downstream tokenization stays unchanged.
        wide_df = self.lake_df[[self.date_col] + selected_cols].copy()
        melted = wide_df.melt(
            id_vars=[self.date_col],
            value_vars=selected_cols,
            var_name="_var_depth_col",
            value_name="_value"
        )
        melted["_var_name"] = melted["_var_depth_col"].map(col_to_var)
        melted[self.depth_col] = melted["_var_depth_col"].map(col_to_depth)

        long_df = (
            melted.pivot(
                index=[self.date_col, self.depth_col],
                columns="_var_name",
                values="_value"
            )
            .reset_index()
        ...
        for var in self.var_names_2D:
            if var not in long_df.columns:
                long_df[var] = np.nan

        long_df[self.depth_col] = pd.to_numeric(long_df[self.depth_col], errors="coerce")
        long_df = long_df[long_df[self.depth_col].notna()].copy()
        if long_df.empty:
            raise ValueError("Wide lake input conversion produced an empty long-form dataframe.")
        
        # CRITICAL FIX: Ensure ALL depths from column structure are present in the DataFrame
        # The pivot may drop (datetime, depth) combinations where ALL variables are NaN
        # But we want to preserve all depths for standardized depth grid across baselines
        
        # Get all unique dates
        unique_dates = sorted(long_df[self.date_col].unique())
        
        # Create complete index with all (date, depth) combinations
        complete_index = pd.MultiIndex.from_product(
            [unique_dates, all_depths_from_columns],
            names=[self.date_col, self.depth_col]
        )
        
        # Reindex to fill in missing (date, depth) combinations with NaN
        long_df = long_df.set_index([self.date_col, self.depth_col])
        long_df = long_df.reindex(complete_index, fill_value=np.nan)
        long_df = long_df.reset_index()
        
        print(f"\nDEBUG: After ensuring all depths are present:")
        print(f"  Shape: {long_df.shape}")
        print(f"  Unique depths: {len(long_df[self.depth_col].unique())}")
        print(f"  Expected: {len(all_depths_from_columns)} depths × {len(unique_dates)} dates = {len(all_depths_from_columns) * len(unique_dates)} rows")

        ordered_cols = [self.date_col, self.depth_col] + self.var_names_2D
        self.lake_df = long_df[ordered_cols].sort_values([self.date_col, self.depth_col]).reset_index(drop=True)
        self.lake_input_format = "wide"
        
        # DEBUG: Check depths after wide-to-long conversion
        unique_depths_after = sorted(self.lake_df[self.depth_col].unique())
        print(f"\nDEBUG: After wide-to-long conversion:")
        print(f"  DataFrame shape: {self.lake_df.shape}")
        print(f"  Unique depths: {len(unique_depths_after)}")
        print(f"  Depths: {unique_depths_after}")
    ...
    def _save_valid_indices(self, flag):
        ...
        # Grid is always unique_datetimes (regular daily sequence)
    ...
    def _extract_dense_y_from_df(self, prediction_date_range, base_dt=None):
        if base_dt is None:
            base_dt = pd.to_datetime(prediction_date_range[0]) if prediction_date_range else None
        else:
            base_dt = pd.to_datetime(base_dt)

        pred_set = set(prediction_date_range)
        pred_df = self.split_df_lake[
            self.split_df_lake[self.date_col].isin(pred_set)
        ].copy()
        pred_df = pred_df.sort_values([self.date_col, self.depth_col]).reset_index(drop=True)
        
        all_values, all_var_ids, all_depth_vals = [], [], []
        all_time_ids, all_time_values, all_datetime_strs = [], [], []
        
        # IMPORTANT: Token ordering must be Variable → Depth → Time
        # This ensures temporal sequences for each (variable, depth) pair are contiguous
        # Expected: (v1,d1,t1), (v1,d1,t2), ..., (v1,d2,t1), (v1,d2,t2), ..., (v2,d1,t1), ...
        
        # Get all unique depths from pred_df (should be all 31 depths for regular grid)
        unique_depths = sorted(pred_df[self.depth_col].unique())
        
        # Iterate: Variable (outer) → Depth (middle) → Time (inner)
        for i, var_idx in enumerate(self.variate_ids_2D):
            var_name = self.var_names_2D[i]
            
            for depth in unique_depths:
                # Get all rows for this (variable, depth) across all times
                # Filter pred_df for this depth
                depth_df = pred_df[pred_df[self.depth_col] == depth]
                
                # Iterate through times in order
                for dt in prediction_date_range:
                    # Find row matching this date and depth
                    row_match = depth_df[depth_df[self.date_col] == dt]
                    
                    if len(row_match) > 0:
                        row = row_match.iloc[0]
                        val = float(row[var_name]) if var_name in row.index else float('nan')
                    else:
                        # No data for this (date, depth) - set to NaN
                        val = float('nan')
                    
                    # Compute time metadata
                    dt_obj = pd.to_datetime(dt)
                    rel_days = (dt_obj - base_dt).days if base_dt is not None else 0
                    doy_norm = (dt_obj.dayofyear - 1) / 365.25
                    
                    # Append token
                    all_values.append(val)
                    all_var_ids.append(var_idx)
                    all_depth_vals.append(depth)
                    all_time_ids.append(rel_days)
                    all_time_values.append(doy_norm)
                    all_datetime_strs.append(dt)
        ...
        return (
            ...
            torch.tensor(all_depth_vals, dtype=torch.float32),
            torch.tensor(all_time_ids, dtype=torch.long),
            torch.tensor(all_time_values, dtype=torch.float32),
            all_datetime_strs,
        )

    def __split__(self, flag='test'):
        # The entire loaded DataFrame is already the desired split (pre-split files).
        # border1/border2 were set to 0/len in __process_data__ and cover the full data.

        # Time features for driver stamp
        ...
        # DEBUG: Check depths in split_df_lake
        split_depths = sorted(self.split_df_lake[self.depth_col].unique())
        print(f"\n{'='*80}")
        print(f"DEBUG: After __split__ (split_df_lake):")
        print(f"{'='*80}")
        print(f"  split_df_lake shape: {self.split_df_lake.shape}")
        print(f"  Unique depths: {len(split_depths)}")
        print(f"  Depths: {split_depths}")
        print(f"  Total unique dates: {len(self.unique_datetimes)}")
        print(f"  First 5 dates: {self.unique_datetimes[:5]}")
        print(f"  Last 5 dates: {self.unique_datetimes[-5:]}")
        print(f"  Date types: {[type(d).__name__ for d in self.unique_datetimes[:3]]}")

        self._build_valid_indices_for_eval(date_grid=self.unique_datetimes)
    ...
    def __process_data__(self):
        ...
        self.prepare_lake_dataframe_format()
        ...
        print(f"\n{'='*80}")
        print(f"DEBUG [{self.lake_name or self.lake_id}]: Depth Grid Information")
        print(f"{'='*80}")
        print(f"  Total unique depths: {self.num_unique_depths}")
        print(f"  Depth values: {self.depth_values}")
        print(f"  Depth range: [{self.min_depth}, {self.max_depth}]")
        print(f"  Total rows in lake_df: {len(self.lake_df)}")
        print(f"  DataFrame shape: {self.lake_df.shape}")
        print(f"{'='*80}\n")
        ...
        # Pre-split files are loaded directly (e.g. *_test.csv), so the entire
        # loaded DataFrame is already the desired split.  No fraction-based
        # slicing is required; borders simply span the full data.
        n_dr = len(self.df_ts)
        n_df = len(df_lake)
        self.border1_DR = 0
        self.border2_DR = n_dr
        self.border1_DF = 0
        self.border2_DF = n_df
    ...
    def __getitem__(self, idx):
        '''
        Token format: (time, depth, variable, value) for consistent embedding.
        Tokens are passed as separate lists instead of stacked tensors.

        idx refers to a position in unique_datetimes (the complete daily grid).
        Context window : observed tokens only (sparse/irregular, as before).
        Prediction window: full dense (T x D x V) grid extracted directly from
            split_df_lake; NaN where unobserved, flat_mask_y = 0 at those positions.
        '''
        # Generate samples based on lake data availability
        if self.valid_idx is not None:
            idx = self.valid_idx[idx]

        # Date grid is always unique_datetimes (regular daily grid after wide→long conversion)
        ...
        # DEBUG: Check prediction date range
        # if idx < 3:  # Only print for first few samples
        #     print(f"\n{'='*80}")
        #     print(f"DEBUG __getitem__ (sample idx={idx}):")
        #     print(f"{'='*80}")
        #     print(f"  self.prediction_len: {self.prediction_len}")
        #     print(f"  self.current_prediction_len: {self.current_prediction_len}")
        #     print(f"  s_begin_grid: {s_begin_grid}, s_end_grid: {s_end_grid}")
        #     print(f"  r_begin_grid: {r_begin_grid}, r_end_grid: {r_end_grid}")
        #     print(f"  len(date_grid): {len(date_grid)}")
        #     print(f"  prediction_date_range length: {len(prediction_date_range)}")
        #     print(f"  prediction_date_range: {prediction_date_range[:5] if len(prediction_date_range) > 5 else prediction_date_range}")

        # Context: only dates that have at least one observed token
        ...
        # Get data for context period (observed tokens only)
        ...
        # Get data for prediction period: full dense (T × D × V) grid from the DataFrame.
        # NaN values are preserved; flat_mask_y marks unobserved positions as 0.
        lake_data_y, lake_var_ids_y, lake_depth_vals_y, lake_time_ids_y, lake_time_values_y, lake_datetime_strs_y = self._extract_dense_y_from_df(
            prediction_date_range, base_dt=base_dt_ctx
        )
        
        # DEBUG: Check what _extract_dense_y_from_df returned
        # if idx < 3:
        #     print(f"  {'='*80}")
        #     print(f"  DEBUG: After _extract_dense_y_from_df (sample idx={idx}):")
        #     print(f"  {'='*80}")
        #     print(f"    lake_datetime_strs_y length: {len(lake_datetime_strs_y)}")
        #     print(f"    unique dates in lake_datetime_strs_y: {len(set(lake_datetime_strs_y))}")
        #     if len(lake_datetime_strs_y) > 0:
        #         unique_dates_y = sorted(set(lake_datetime_strs_y))
        #         print(f"    unique dates: {unique_dates_y[:5] if len(unique_dates_y) > 5 else unique_dates_y}")
        #         print(f"    lake_data_y shape: {lake_data_y.shape}")
        #         print(f"    Expected tokens: {len(prediction_date_range)} dates × {len(self.depth_values)} depths × {len(self.variate_ids_2D)} vars = {len(prediction_date_range) * len(self.depth_values) * len(self.variate_ids_2D)}")
        #         print(f"    Actual tokens: {len(lake_datetime_strs_y)}")
